...
- (void)keyboardWillShow:(NSNotification *)inNotification {
    if ([self shouldMoveForKeyboard]) {
        ...
        CGFloat padding = self.isCreateHaggleOffer ? 75 : 20;
        ...
        if (IS_IPAD) {
            ...
            if (UIDeviceOrientationIsLandscape([UIDevice currentDevice].orientation))
            {
                ...
                padding = padding - keyboardHeight / 10 * 8;
            }
        }
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    ...
    if (!nextCell) {
        ...
        if ([nextCell isKindOfClass:[FormTextInputCell class]]) {
            [((FormTextInputCell*)nextCell).textField becomeFirstResponder];
